...
(function($, window, document, undefined) {
    ...
    DatePaginator.defaults = {
        ...
        text: 'ddd<br/>Do',
        textSelected: 'dddd<br/>Do, MMM YYYY',
    ...
    DatePaginator.prototype = {
        ...
        _clickedHandler: function(event) {
            ...
            if (classList.indexOf('dp-nav-left') != -1) {
                /*var selectedDateDay = this.options.selectedDate.startOf('day');
                var startOfWeek = this._isStartOfWeek(this.options.selectedDate);
                if(startOfWeek)
                {
                	this._back();
                	this._back();
                }*/
                this._back(getNoOfDays(-1));

            } else if (classList.indexOf('dp-nav-right') != -1) {
                /*var selectedDateDay = this.options.selectedDate.startOf('day');
                var mm = selectedDateDay._d.__proto__.getDay();
                alert(mm);*/
                this._forward(getNoOfDays(1));
            } else if (classList.indexOf('dp-item') != -1) {
                this._select(target.attr('data-moment'));
            }
        },
        _setNoOfDays: function(noOfDays) {
            this.options.noOfDays = noOfDays;
        },
        ...
        _back: function(days) {
            this._setSelectedDate(this.options.selectedDate.clone().subtract('day', days));
            this._render();
        },

        _forward: function(days) {
            this._setSelectedDate(this.options.selectedDate.clone().add('day', days));
            this._render();
        },
